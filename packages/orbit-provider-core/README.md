...
It contains fixed provider profiles, request and response conversion, model discovery, retries, reasoning-state replay, and normalized token usage for both Chat Completions and Responses APIs. Credentials, OAuth, billing, pricing, UI, tools, and skills are supplied by the host application and are not part of this package.
