...
export function appDirectories(env: NodeJS.ProcessEnv = process.env): AppDirectories {
...
export async function readJson<T = unknown>(file: string, fallback?: T): Promise<T> {
  try {
    ...
    if (errorCode(error) === 'ENOENT') return fallback as T
    throw error
  }
}
...
export function boundedString(value: unknown, label: string, maximum = 16_000): string {
...
export async function collectStream(stream: any, maximum: number): Promise<Buffer> {
  if (!stream) throw new Error('Response body is missing')
  const chunks: Buffer[] = []
  let total = 0
  const add = (value: Uint8Array | ArrayBuffer) => {
    const chunk = Buffer.from(value instanceof ArrayBuffer ? new Uint8Array(value) : value)
