...
export function secretLikeFileName(name) {
...
export function findHighConfidenceSecrets(value) {
  ...
  for (const match of text.matchAll(ASSIGNED_SECRET)) {
    if (!placeholder(match[1])) findings.push('assigned credential')
  }
...
export function redactHighConfidenceSecrets(value) {
  ...
  ASSIGNED_SECRET.lastIndex = 0
  text = text.replace(ASSIGNED_SECRET, (match, secret) => placeholder(secret) ? match : match.replace(secret, '[redacted-assigned-credential]'))
  CREDENTIAL_URL.lastIndex = 0
  return text.replace(CREDENTIAL_URL, '[redacted-credential-url]')
}

export function scanSecretBytes(bytes) {
