...
test('protects management APIs and isolates preview content on another origin', async (t) => {
  ...
  const publishDialogHtml = await fetch(started.origin).then((r) => r.text())
  assert.match(publishDialogHtml, /id="publish-locales"/)
  assert.match(publishDialogHtml, /class="locale-chips"/)
  assert.match(publishDialogHtml, /In-game languages/)
  const appSourceForLocales = await fetch(`${started.origin}/app.js`).then((r) => r.text())
  assert.match(appSourceForLocales, /PUBLISH_LOCALE_CHOICES/)
  assert.match(appSourceForLocales, /renderPublishLocaleChips/)
  assert.match(appSourceForLocales, /extraLocales: publishLocales/)
})
