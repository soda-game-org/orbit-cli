...
test('public release audit validates the final npm pack manifest', async () => {
  const { stdout, stderr } = await execFileAsync(process.execPath, ['--import', tsxImport, 'scripts/audit-public.mts'], {
    cwd: root,
    encoding: 'utf8',
    maxBuffer: 4 * 1024 * 1024,
  })
  assert.equal(stderr, '')
  assert.match(stdout, /Public release audit passed: the checkout, reachable Git history and \d+ packaged files contain no maintained JavaScript source/)
})
...
test('public release audit rejects a credential removed from the current tree but retained in reachable history', async (t) => {
  ...
  await assert.rejects(execFileAsync(process.execPath, ['--import', tsxImport, 'scripts/audit-public.mts'], {
    cwd: checkout,
    encoding: 'utf8',
    maxBuffer: 4 * 1024 * 1024,
  }), (error) => {
